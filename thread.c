...
bool
less (struct list_elem *new, struct list_elem *elem, void *aux UNUSED)
{
  return list_entry (new, struct thread, elem) -> priority > list_entry (elem, struct thread, elem) -> priority;
}
...
thread_wake (int64_t ticks) 
{
  ...
  while (e != list_end (&blocked_list)) { 
    blocked = list_entry(e, struct thread, elem);
    if (blocked -> wake <= ticks) {
      e = list_remove (e);
